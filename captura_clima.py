...
def capturar():
    ...
    # 1. SISTEMA DE INSISTENCIA: Intenta hasta 3 veces si la página de Mercobras no responde
    for intento in range(3):
        try:
            ...
            exito_conexion = True
            break # Si tuvo éxito, rompe el ciclo de intentos
        except:
            print(f"Intento {intento+1} fallido. Reintentando...")
            time.sleep(5) # Espera 5 segundos antes de reintentar

    # 2. FUNCIÓN EXTRACTORA 3D
    def extraer(seccion, etiqueta, ocurrencia=1, unidad=None, es_hora=False):
        if not exito_conexion: return "ERROR" # Si falló la conexión, anota ERROR
    ...
    # 3. MAPEO DE TODOS LOS DATOS SOLICITADOS
    m_dir = re.search(r"Sector\s*([A-Z]+)", html_limpio) if exito_conexion else None

    datos = {
        "Fecha_Hora": fecha_str,
        
        # TEMPERATURA (Mínimas, Máximas y Horarios)
        ...
        # HUMEDAD (Mínimas, Máximas y Horarios)
        ...
        # VIENTO (Velocidad, Ráfagas y Horarios)
        "Viento_Vel_Actual_kmh": extraer("VIENTO", "elocidad", 1, "km/h"),
        "Viento_Direccion": m_dir.group(1) if m_dir else ("ERROR" if not exito_conexion else "S"),
    ...
    # 4. GUARDADO A PRUEBA DE FALLAS
    try:
        df_nuevo = pd.DataFrame([datos])
        
        # Evitamos problemas de formato limpiando el archivo si tiene basura
        if os.path.exists(archivo) and os.path.getsize(archivo) > 0:
            try:
                ...
                # Si el archivo viejo estaba roto, lo pisa y empieza fresco
                df_nuevo.to_csv(archivo, index=False, encoding='utf-8-sig')
        else:
            df_nuevo.to_csv(archivo, index=False, encoding='utf-8-sig')
            
        print(f"GRABADO OK. Estado conexión: {'EXITO' if exito_conexion else 'FALLÓ'}")

    except Exception as e:
        print(f"Error al grabar CSV: {e}")

    # 5. DESCARGAR GRÁFICOS (Solo si hubo conexión exitosa)
    if exito_conexion:
